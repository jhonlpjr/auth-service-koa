import { MfaFactorsRepository } from '../../../../application/ports/repositories/mfa-factors.repository';
...
export class PgMfaFactorsRepository implements MfaFactorsRepository {
  async getPending(userId: string, type: MfaTypes): Promise<UserMfaFactor | null> {
  ...
  async createPending(userId: string, type: MfaTypes, secretCiphertext: string): Promise<UserMfaFactor> {
    const id = randomUUID();
    const now = new Date();
    const result = await PostgresDB.query(
  ...
  async activateFactor(userId: string, type: MfaTypes): Promise<void> {
    await PostgresDB.query(
      `UPDATE user_mfa_factors SET status=ACTIVE_STRING, updated_at=$1 WHERE user_id=$2 AND type=$3 AND status='pending'`,
      [new Date(), userId, type]
    );
  }

  async getActive(userId: string, type: MfaTypes): Promise<UserMfaFactor | null> {
    const result = await PostgresDB.query(
      `SELECT * FROM user_mfa_factors WHERE user_id=$1 AND type=$2 AND status=ACTIVE_STRING`,
      [userId, type]
    );
    return result.rows[0] ? (result.rows[0] as UserMfaFactor) : null;
  }
